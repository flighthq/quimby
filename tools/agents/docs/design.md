...
- **A `.quimby/` standing where the storage symlink belongs is healed when it is debris.** `state.yaml` is the discriminator: without one there is no workspace there, whatever else the directory holds, so quimby carries any stray `local.yaml` into storage, clears the rest, and links. This is not an exotic state — deleting `.quimby` deletes only the symlink, and a running `quimby serve` recreates the directory within one poll cycle (reconciling status mirrors `mkdir -p`s `.quimby/agents/<id>/status` for every agent), so the recovery command refused because of a directory the server had just recreated. A stray directory that _does_ carry a `state.yaml` is still refused, naming both paths.
...
The recipient check consults **two** sources, because neither alone is complete: the roster baked in when the tool was last rendered, and the live `status/` mirror the server reconciles each poll cycle. `quimby add` rewrites only the new agent's scaffold, so every existing agent's baked roster predates the addition until its next launch or `quimby sync` — consulting it alone made `handoff` refuse an agent that `peers` was listing. A name in either source is real; a name in neither is the typo the check exists to catch, and the error prints the union so what it lists matches what it accepts.
...
3. Mirror the cycle's changed statuses into **every other agent's** `status/<name>.md` (local or remote) — no subscription filter. Delivery is **batched per recipient** (one call carrying every changed peer) and recipients are written concurrently, so the N×N fan-out costs N round trips rather than N². Serialized one-file-per-round-trip delivery made an 8-agent SSH fleet spend ~16s of every 5s cycle here, overrunning it and delaying the integration, base delivery and auto-dispatch below. Reporting is one line per source agent (`[builder] status → 7 peer(s)`), with any failed recipient named individually — condensing the success path must not condense away a miss. Each snapshot carries an `Unmerged:` header line — the commits and files that exist only in that agent's clone — because that is the direction of base staleness an agent cannot see for itself (`agent.sh` reports when the base is ahead of _it_, never when a _peer_ is ahead of the base). It is measured only for an agent whose status actually changed, so it stays off the hot path, and it is omitted rather than reported as zero when the repo cannot be read — absence means "not measured", `0` is a claim
4. When the workspace declares `integrate:`, pull that agent's committed work across the boundary — the automated twin of `quimby merge <agent> --commits`. Opt-in and off by default: this is the only server behavior that **writes the user's real repository**, so it has to be stated in config rather than inferred from a flag. The mode is **pinned to `commits`**, not configurable — it is the only mode that is idempotent (patch-id deduped), never fabricates a commit message, and leaves the uncommitted remainder on the agent, which is also what lets it pull from a worker that is still typing. A conflict **never leaves the host mid-merge**: the tentative merge is aborted (repo untouched, no `MERGE_HEAD`), the parcel discarded, and the agent handed the forced `rebase onto origin/<ref>` request, since it has the code context. **One tree is attempted once** — keyed on the integrator's content hash, so neither a conflict nor a wedged repo becomes a nudge loop, while genuinely new commits are carried afresh. A dirty host repo is reported once and waited on, never touched. It runs **before** the base delivery below on purpose: a landing moves the host tip, so delivering afterwards carries the integrator's work out to the whole fleet in the _same_ cycle. That ordering is also why this cannot be a separate process — it would be running git in the agents' clones while the base delivery syncs those same clones, and the merge is precisely what triggers that sync
5. Resolve every watched `syncRef` in the host repo; when a tip has moved, deliver the base to every agent (the automatic twin of `quimby sync --all`) — skipped under `--no-sync`. Nothing else in the running system watches the host repo, so before this the base moved only when a human typed `quimby sync`, and a fleet whose work lands through one integration agent sat waiting on a base that was already merged. It **never rewrites an agent**: this is the ordinary deferring sync, so it moves the `quimby/base` tag and fast-forwards only an agent with a clean tree and no commits of its own — anything with work in flight is reported as delivered-not-applied and applies the base itself with `./agent.sh rebase`. That is what makes it safe to run unattended. A **first sighting delivers too**, rather than only recording the tip: the alternative leaves a tip that moved while the server was down invisible until it happens to move again, which is the common shape of the staleness this watches for (merge, then start the server), and the sync is idempotent so a current fleet costs a no-op per agent. Reporting names each agent that still owes itself the advance and each whose sync was skipped, while the agents that simply landed on the new base are a count. A ref that stops resolving is named once, on the transition, and delivers nothing; a failed delivery leaves the tip unrecorded so the next cycle retries
6. Scan each agent's `out/queued/`; auto-dispatch any parcel whose newest mtime was unchanged since the previous cycle (settled), then nudge the recipient — skipped entirely under `--no-dispatch`
7. Re-announce any parcels an idle agent still hasn't read (spaced by 10 minutes, and at most 3 **delivered** reminders for an unchanged inbox before it reports the agent as stuck) — the safety net that keeps an unattended fleet from stalling on a lost wake; skipped under `--no-dispatch`. A parcel is never announced **on first sighting**: the sweep is the net for a wake that was lost, and it cannot tell one from a wake another carrier just sent (a CLI `delegate`/`handoff`, or step 6's own wake still inside its bundle window) or deliberately withheld (an advisory parcel under `nudge: directed`). It records the sighting and lets the carrier's rule govern the fresh parcel; anything still unprocessed one interval later is the sweep's business. A reminder the §7 guard **held** delivered nothing, so it neither consumes the cap nor starts the interval: it is retried on the next cycle, landing about one cycle after the human stops typing. Without that, sitting in an agent's pane burned all three reminders without one reaching it, and quimby then went silent and reported a healthy agent as stuck
8. When `pool.idleTimeout` is configured, reap this project's agent sessions idle past it (see [The Agent Pool](#the-agent-pool)) — skipped when unset (the default), and it never touches an attached session
...
This refresh runs whether or not the advance succeeded — an agent whose sync fails is skipped by `--all` and is exactly the one most likely to be carrying a stale roster, so freezing its docs because the base could not move would compound one problem with a second, invisible one. The write stays best-effort (it never fails a sync) but is reported when it fails, since a silent swallow made `sync`'s success line no evidence that anything was rewritten.
...
```yaml
integrate:
  from: integration # the agent whose committed work is pulled across the boundary
  branch: quimby/landed # optional: park it on a branch instead of the tracked one
```

Unset (the default) means the server never merges. When set, a running `quimby serve` lands that agent's committed work whenever it commits something new, and — because integration runs before the base delivery in the same cycle — the result reaches every other agent's `quimby/base` on that same tick.

Three properties make it safe to leave running:

- **`commits` mode, pinned.** Only committed work crosses; the uncommitted remainder stays on the agent, so integrating never interrupts a worker mid-edit and never invents a commit message. It is idempotent, so a re-gather lands nothing new.
- **A conflict routes back to the agent.** The tentative merge is aborted — your repo is left exactly as it was — the parcel is discarded, and the agent is handed the forced `rebase onto origin/<ref>` request. Nothing retries until its tree changes, so a conflict never becomes a nudge loop.
- **Your working tree is never touched.** A dirty host repo defers the landing (reported once) rather than stashing anything.

With `branch:` the work is parked off the agent's `syncRef`, so — exactly as with `merge -b` — the seed post-advance stands down and the fleet's base does not move. That is the "review before it reaches the others" shape; omit it for the "keep the fleet moving" one.

`merge` itself is unchanged and remains the verb for everything else. This automates one narrow, idempotent case; it does not make the boundary automatic.
