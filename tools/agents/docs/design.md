...
`merge` does this automatically on a clean, committed merge (`--no-advance` opts out) by running `sync -f` for the agent — but only when it is provably lossless:
