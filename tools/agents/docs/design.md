...
5. Re-announce any parcels an idle agent still hasn't read (spaced by 10 minutes, and at most 3 **delivered** reminders for an unchanged inbox before it reports the agent as stuck) — the safety net that keeps an unattended fleet from stalling on a lost wake; skipped under `--no-dispatch`. A reminder the §7 guard **held** delivered nothing, so it neither consumes the cap nor starts the interval: it is retried on the next cycle, landing about one cycle after the human stops typing. Without that, sitting in an agent's pane burned all three reminders without one reaching it, and quimby then went silent and reported a healthy agent as stuck
