...
### Agent-side mechanics (`quimby-agent.sh`)

The comms conventions above are enacted **inside** the sandbox by the agent, and several of the steps are fiddly and silent-failure-prone: the atomic author-then-publish move (`out/draft/<recipient>` → `out/queued/<recipient>`), the exact `quimby-attest` block with a correctly-keyed `atCommit`, the inbox `received → processed` lifecycle. Quimby scaffolds a small **agent-side mailbox tool** — `quimby-agent.sh` (with a Windows `.cmd` twin) — into every agent directory so the agent runs these mechanics reliably instead of hand-formatting them each time.
...
```
quimby-agent.sh handoff <recipient> [-m msg] [--attach agent] [--file path] [--draft]
quimby-agent.sh publish <recipient>          # publish a parcel drafted with --draft
quimby-agent.sh inbox [show <p> | done <p>]  # list / read / mark-processed delivered parcels
quimby-agent.sh attest --command CMD --result pass|fail [--summary S]   # append a quimby-attest block, atCommit auto-filled from HEAD
quimby-agent.sh peers [name]                 # list peer status mirrors, or read one
```
...
- **It never crosses the boundary.** `quimby-agent.sh` only ever writes the agent's **own** workspace (its `out/draft` → `out/queued`, its `status.md`, its inbox). Carrying a parcel to a peer, or out to the user's repo, stays the host's job (`quimby dispatch`/`merge`). The agent tool and the host courier stay cleanly separated.
