...
**Quimby's answer**: Session-less model. Assignments go in, handoffs come out. No persistent context.
...
**Quimby's answer**: Agents are runtime-agnostic. The protocol (assignment.md, status.md, handoffs) is the same regardless of what agent or model runs inside the sandbox.
