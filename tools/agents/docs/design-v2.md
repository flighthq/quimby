...
quimby set <worker> [-r <rt>] [-a <agent>] [-H <host>] [--port <n>] [-s <ref>]  Update worker config (--sync retargets advance)
...
- **`seedCommit`** (mirrored by the `quimby/seed` tag) — the base the worker's work is measured from. All pack diffs are `git diff quimby/seed`.
...
The apply target is independent of `syncRef` — `quimby apply <pack> -t <branch>` lands a pack wherever you choose, regardless of where its seed was derived from.
