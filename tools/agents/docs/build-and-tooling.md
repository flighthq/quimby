...
- Concrete agent state is **per-agent** in `.quimby/state.yaml`. A tracked `quimby.yaml` carries auditable shared intent when present and wins for the roles, presets, runtime profile launch fields, layouts, and defaults it defines. Ignored `.quimby/local.yaml` carries private per-checkout details, local-only additions, and machine-specific fills. `config` is still an interactive `set` for an existing agent; `add --role` and `up <preset>` apply saved creation defaults.
...
**tmux is universal**: every agent — local or SSH — always runs in its own persistent tmux session; there is no foreground path. `quimby run` wraps the agent in `tmux new-session -A -s <tmuxSessionName>` (attach-or-create, so it grabs the one canonical session wherever launched) and `quimby start` uses `new-session -A -d` (detached, headless); either way `run` enrolls the `tmux` field on `AgentState` so `nudge`/`list` recognize the session. The session name derives from stable IDs (`qb-<projectId[:8]>-<agentId[:8]>`), so it survives renames. `run` and `start` share one launch-prep helper (`apps/cli/src/launch.ts`) — SSH sync + lazy init, runtime spec, bundled tmux config — differing only in attach-vs-detached, and `quimby list` reports each agent's live session state (`getAgentSessionState` → running / attached / stopped). Running multiple agents (`quimby run <a> <b>`) opens a **dashboard**: a stateless viewport session that `link-window`s each local agent's own window in as a tab (SSH tabs ssh-attach the remote session, `host` is a plain shell). Because it only borrows a view, the per-agent sessions outlive it — the dashboard is rebuilt on each run, tabs are numbered from 0, and per-tab activity/silence highlighting is set as window options (shared with the linked agent session but invisible there, since that session's plain `window-status-format` doesn't react).
