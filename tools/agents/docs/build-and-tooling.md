...
- Concrete agent state is **per-agent** in `.quimby/state.yaml`, while ignored `.quimby/local.yaml` carries reliable per-checkout roles, recipes, runtime profiles, and layouts. A tracked `quimby.yaml` is optional for team-safe shared intent only. `config` is still an interactive `set` for an existing agent; `add --role` and `up <recipe>` apply saved creation defaults.
