...
- Done: process-wide crash instrumentation was removed. VS Code extensions share one extension-host process, so `process.on('unhandledRejection')` / `process.on('uncaughtException')` made unrelated extension failures look like Quimby failures in the Quimby output channel.
- Done: a status-bar item (`$(shield) Quimby` idle / `$(layout) Quimby: <name>` when a layout is open) proves the extension is live in a given window. Idle opens the single-agent picker; layout open/restore remains explicit so startup does not accidentally launch every saved-layout agent.
