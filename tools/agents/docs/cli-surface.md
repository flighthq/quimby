...
quimby merge <agent> [--commits|--patch] [--3way] [-b] [-t] [-m "..."] [--sync <ref>|--no-sync]   Merge the agent's work into your repo (the boundary); by default syncs the agent onto the target first (so base-drift conflicts resolve on the agent, not your repo), then squashed authors one commit (editor, or -m), then advances the seed on a clean landing (--sync <ref> aligns onto <ref> before and retargets the sync ref after; --no-sync leaves the agent's base untouched at both ends)
...
- `--sync` / `--no-sync` (merge — governs the agent's base alignment at **both ends**, on by default: syncs the agent onto the merge target _before_ landing (the safe rebase, so base-drift conflicts resolve on the agent, off your repo) and advances the seed _after_ a clean, committed landing — both gated on merging into the branch the agent tracks; `--sync <ref>` aligns onto `<ref>` before and retargets the agent's sync ref to `<ref>` after; `--no-sync` leaves the agent's base untouched at both ends, i.e. the raw merge with host-side conflict resolution). One optionally-valued `--sync` — bare/absent aligns before + advances after, a ref retargets, `--no-sync` skips — so `--sync` means the same thing on add/set/assign/merge.
