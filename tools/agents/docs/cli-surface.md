...
quimby sync <agent...> [--all] [-f] [--base <ref>] [--current]   Sync agent(s) to their base, keeping work (-f hard-resets; --base/--current retarget); also re-renders each agent's Quimby-tier scaffold (CLAUDE.md/AGENTS.md + agent.sh) onto its dir, so a quimby upgrade reaches in-flight agents without killing their session/sandbox (ingested at the agent's next context reset), and re-resolves each agent's coordination edges (directs/escalatesTo) from quimby.yaml, so a graph edit takes effect on the next dispatch with no rebuild
