...
quimby merge <agent> [--commits|--patch|--squashed] [--3way] [--preview] [-b] [-t] [-m "..."] [--sync <ref>|--no-sync] [--default [--global]]   Merge the agent's work into your repo (the boundary); --preview is a read-only dry run (mode, commits, diffstat, self-check, behind-base) that crosses nothing; mode is an explicit --commits/--patch/--squashed, else the configured mergeMode default, else squashed; by default syncs the agent onto the target first (so base-drift conflicts resolve on the agent, not your repo), then squashed authors one commit (editor, or -m), then advances the seed on a clean landing (--sync <ref> also retargets the sync ref as it advances; --no-sync leaves the agent's base untouched at both ends; --default persists the chosen mode as the repo default, --global to user config)
