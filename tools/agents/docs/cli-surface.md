...
quimby sync <agent...> [--all] [-f] [--base <ref>] [--current]   Deliver the base to agent(s) and apply it where applying is not a rewrite. Always moves the `quimby/base` tag (one ref write — never touches HEAD, the index, or the working tree, so it is safe against in-flight work and runs even on a wedged repo). Then fast-forwards an agent with NO commits and a clean tree; an agent with commits or a dirty tree is left untouched and reported as `base delivered … not applied` — it applies the base itself with `./agent.sh rebase`, because only the agent knows when its tree is at a safe point. -f hard-resets (the escape hatch, and the only host path for a stopped agent); --base/--current retarget. Also re-renders each agent's Quimby-tier scaffold (CLAUDE.md/AGENTS.md + agent.sh + the advisory pre-commit hook) onto its dir, so a quimby upgrade reaches in-flight agents without killing their session/sandbox (ingested at the agent's next context reset), and re-resolves each agent's coordination edges (directs/escalatesTo) from quimby.yaml, so a graph edit takes effect on the next dispatch with no rebuild
