...
export function renderTmuxConfig(): string {
  return (
    [
      ...
      '# Re-fit a window whose app is drawing at a stale size: nudge the width by one to force a',
      '# real SIGWINCH (a repaint alone will not make a TUI re-layout), then UNSET the window-local',
      '# option — `resize-window` pins the window to `window-size manual`, so skipping the unset',
      '# would leave it permanently ignoring its clients. Backgrounded so the settle beat between',
      '# the two steps does not block the server; without it they coalesce and the pty is left one',
      '# column narrower than the window.',
      '# Both steps target #{window_id}, expanded ONCE against the pane the key was pressed in. A',
      '# pane-process variable cannot be used here (run-shell does not inherit one) and an empty',
      '# -t does not error, it means "current window" — so a tab switch during the settle beat put',
      '# the pin on one window and the unpin on another, stranding the first at manual size.',
      'bind = run-shell -b \'tmux resize-window -t "#{window_id}" -L; sleep 0.4; tmux set -w -t "#{window_id}" -u window-size\'',
