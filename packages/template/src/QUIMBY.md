...
**Closing a parcel is a statement that you are finished with it, which you cannot be if you never opened it.** `inbox done --all` exists so a deep tray is clearable, and it closes only what you have actually read; anything unopened it keeps and names. If `./agent.sh inbox` reports parcels _closed without ever being read_ — the state a careless bulk close leaves behind — that is real work nobody has seen, and nothing else will announce it: the host's reminder sweep watches only the received tray. Read one with `inbox show <parcel>`, and `inbox reopen <parcel>` to put it back where you will act on it.

When you see it, finish the thought you are on, **commit**, then run `./agent.sh rebase`. It refuses on a dirty tree on purpose — and when it does, it names the uncommitted files and marks the ones the incoming base also changes. Those marked files are the trap: discarding one reverts work that already landed, with no conflict to warn you. Commit your own paths rather than clearing the tree to get past the refusal. Replaying over uncommitted work is how a pre-sync copy of a file gets restored on top of a peer's just-landed change — silently, with no conflict — and you then commit that revert inside a commit named after your own feature. Nothing downstream catches it: to git it looks like a deliberate edit.
