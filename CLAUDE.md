...
- `packages/errors/` — `@quimbyhq/errors` — error taxonomy (QuimbyError, GitError, AgentError, HandoffError, ConflictError, SyncConflictError — the latter carries `agentClean` so `merge` can tell a rolled-back-clean pre-sync rebase conflict, which it falls back past to the net boundary merge, from a wedged repo it must block on)
...
- `packages/session/` — `@quimbyhq/session` — a live agent's tmux session: waking it (nudgeAgentSession, hasAgentSession) and reading its state (getAgentSessionState → running/attached/stopped); reused by the CLI nudge/assign/dispatch/handoff/start/stop/list and the server's auto-dispatch. Also the socket-wide primitives `listQuimbyTmuxSessions`/`killQuimbyTmuxSession` (every session on the shared `tmux -L quimby` server, by name) that back the agent pool
- `packages/pool/` — `@quimbyhq/pool` — the machine-wide agent pool: `getPoolInventory` joins every session on the shared tmux socket to its owning workspace (agent/dashboard/view/orphan, idle age), `selectPrunablePoolSessions`/`prunePoolSessions` reap idle/orphaned sessions (never attached), `getPoolCapacity`/`poolCapacityWarning` (advisory `pool.maxLive`) and `getPoolIdleTimeoutMs` (opt-in `pool.idleTimeout` auto-reap). Backs `quimby sessions`/`sessions prune`, the launch-time capacity warning, and the server's auto-reap. Sits above session+workspace in the DAG
- `packages/workspace/` — `@quimbyhq/workspace` — `.quimby/` state lifecycle (resolve/ensure/load/save, migrations — including the one-time reshape of a legacy `inbox/`+`outbox/` mailbox into the `handoff/` tree, and scrubbing the dropped `subscriptions` map, on state load) + remote reconnect (`remoteAdopt.ts`): `resolveWorkspace`/`ensureWorkspace` **adopt-or-create** — before minting a fresh project id they scan bound host aliases for a remote workspace matching this repo's git origin and adopt the unique match, so a lost `.quimby/` reconnects instead of orphaning the remote. `adoptRemoteWorkspace` (silent, bound-only) and the CLI `restore` share one `reconstructRemoteWorkspace` core; `pruneRemoteWorkspaces` is the explicit remote-GC. Depends on `@quimbyhq/transport` for the SSH scan (prompting stays in the CLI)
- `packages/agent/` — `@quimbyhq/agent` — agent lifecycle (add, remove, rename, rebuild, sync targets) + operations: `assignAgentTask` (write→sync→nudge directive), `syncAgents` (multi-agent sync with outcome classification), `rebaseAgentOntoBase` (the `--rebase` beforeStage seam). `syncAgent` drives the pure `runSyncAlgorithm` (in `syncAlgorithm.ts`) over a `RepoSyncOps` backend — local (git CLI) and SSH (git over transport) share one stash/rebase/reset/tag algorithm, tested against a fake. After advancing the base, `syncAgent` also re-renders the agent's Quimby-tier scaffold (`writeAgentInstructions`/`writeRemoteAgentInstructions` — CLAUDE.md/AGENTS.md + agent.sh) best-effort, so a quimby upgrade reaches in-flight agents without a session/sandbox kill (they ingest at the next context reset); it never touches assignment.md/status.md/the mailbox. Agent provisioning is factored into primitives too: `cloneAndSeedAgentRepo`/`writeAgentScaffold` (local) and the exported `cloneAndSeedRemoteAgentRepo`/`writeRemoteAgentScaffold`/`renderRemoteMailboxMigration` (SSH, reused by `@quimbyhq/launch`'s `prepareSshLaunch`), so `addAgent`/`rebuildAgent`/first-run init share one clone+seed+scaffold path (scaffolding the `handoff/` mailbox tree, and migrating a legacy remote mailbox lazily on next run)
...
- `packages/server/` — `@quimbyhq/server` — HTTP server + status poller + outbox auto-dispatch + opt-in idle-session auto-reap (`autoreap.ts`, gated on `pool.idleTimeout`) + client (CLI → server API); takes a `Reporter` for all output

Dependency flow is a DAG: leaves (types, errors, utils, paths, reporter, template) → git/transport/runtimes/session → workspace → pool → agent/handoff → launch → server → apps/cli.
