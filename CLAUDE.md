...
- `packages/workspace/` — `@quimbyhq/workspace` — `.quimby/` state lifecycle (resolve/ensure/load/save, migrations — including the one-time reshape of a legacy `inbox/`+`outbox/` mailbox into the `handoff/` tree, and scrubbing the dropped `subscriptions` map, on state load)
