...
/**
 * Render an agent's merge-state as a one-line signal for `diff`/`status`/`list`:
 * `"3 files, 2 commits, +140/-12 — not yet merged"`, or `"synced — no unmerged work"`
 * when nothing sits on the seed. A null summary (unreadable repo) reads as unavailable.
 * Colorless — callers add any emphasis.
 */
export function formatWorkSummary(summary: Readonly<AgentWorkSummary> | null): string {
  ...
  if (files === 0 && commits === 0) return 'synced — no unmerged work'
  const parts: string[] = []
