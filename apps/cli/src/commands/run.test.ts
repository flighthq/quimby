...
describe('runRunCommand', () => {
  ...
  it('styles in-dashboard agent tabs with colored state accent bars (no dots or partial circles)', async () => {
    ...
    expect(flat).toContain('#[fg=colour240]▎#[fg=colour244] #W ') // idle: grey bar + padded title
    expect(flat).toContain('#[fg=colour240]▎#[fg=colour231] #W ') // selected: both spaces are highlighted
